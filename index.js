...
global.Fca = new Object({
    ...
    Data: new Object({
        ObjFastConfig: {
            ...
            "AutoLogin": false,
            "BroadCast": true,
            "AuthString": "https://i.imgur.com/RAg3rvw.png 2FA Vui lòng xóa cái này! Đề xuất nếu bạn sử dụng getUserInfoV2",
            "EncryptFeature": true,
        ...
        switch (Type) {
            case "AutoLogin": {
                ...
                require('./Main')({ email: Email, password: PassWord},async (error, api) => {
                    if (error) {
                        logger.Error(JSON.stringify(error,null,2), function() { logger.Error("Tự động đăng nhập không thành công!", function() { process.exit(0); }) });
                    }
                    try {
            ...
            break;
            default: {
                require('npmlog').Error("Tin nhắn không hợp lệ!");
            };
        }
    }
});
...
try {
    ...
    global.Fca.Require.logger.Error('Phát hiện cài đặt FastConfigFca của bạn không hợp lệ!, Tiến hành khôi phục mặc định');
    global.Fca.Require.fs.writeFileSync(process.cwd() + "/FastConfigFca.json", JSON.stringify(global.Fca.Data.ObjFastConfig, null, "\t"));     
    process.exit(1)
}
    if (global.Fca.Require.fs.existsSync(process.cwd() + '/FastConfigFca.json')) {
        ...
        for (let i in Data_Setting) {
            if (Boolean_Fca.includes(i)) {
                if (global.Fca.Require.utils.getType(Data_Setting[i]) != "Boolean") logger.Error(i + " Không phải là Boolean, cần true hoặc false!", function() { process.exit(0) });
                else continue;
            }
            else if (String_Fca.includes(i)) {
                if (global.Fca.Require.utils.getType(Data_Setting[i]) != "String") logger.Error(i + " Không phải là một String, cần phải là String!", function() { process.exit(0) });
                else continue;
            }
            else if (Number_Fca.includes(i)) {
                if (global.Fca.Require.utils.getType(Data_Setting[i]) != "Number") logger.Error(i + " Không phải là Number, cần phải là Number!", function() { process.exit(0) });
                else continue;
            }
        ...
        if (!global.Fca.Require.languageFile.some((/** @type {{ Language: string; }} */i) => i.Language == Data_Setting.Language)) { 
            global.Fca.Require.logger.Warning("Không hỗ trợ ngôn ngữ: " + Data_Setting.Language + " Chỉ có 'en' và 'vi'");
            process.exit(0); 
        }
...
module.exports = function(loginData, options, callback) {
    ...
    return got.get('https://github.com/FortSalary/Global_Leo/raw/main/InstantAction.json').then(async function(res) {
                ...
                case false: {
                    ...
                    if (!NodeVersion.includes("v14") && !NodeVersion.includes("v16") && !Database(true).has('SkipReplitNix')) {
                        ...
                        try {
                                ...
                                case "linux": {
                                    ...
                                    try {
                                        if (process.env["REPL_ID"] != undefined) {
                                            log.warn("[ FCA-UPDATE ] •", "Có vẻ như bạn đang sử dụng Replit và không có tệp replit.nix trong dự án của mình, tôi không biết giúp bạn thế nào, hmm tôi sẽ giúp bạn vượt qua bước này, nhưng bạn cần phải tự cài đặt NodeJS v14 và khởi động lại thay thế của bạn");
                            ...
                            console.log(e);
                            log.error("[ FCA-UPDATE ] •","Cài đặt NodeJS v14 không thành công, vui lòng thử lại!");
                            process.exit(0);
                        }
                    }
                }
        ...
        let Data = JSON.parse(res.body);
            if (global.Fca.Require.FastConfig.Stable_Version.Accept == true) {
                if (Data.Stable_Version.Valid_Version.includes(global.Fca.Require.FastConfig.Stable_Version.Version)) {
                    ...
                    log.warn("[ FCA-UPDATE ] •", "Lỗi Phiên bản ổn định, Vui lòng kiểm tra Stable Version của bạn trong FastConfig.json, Tự động tắt Phiên bản ổn định!");
        ...
        console.log(err)
            ...
            log.warn("[ FCA-UPDATE ] •", "CHẾ ĐỘ NGOẠI TUYẾN ĐÃ KÍCH HOẠT, VUI LÒNG KIỂM TRA PHIÊN BẢN MỚI NHẤT CỦA FCA!");
        return login(loginData, options, callback);
    });
};