...
if (!fs.existsSync(process.cwd() + '/Leo_Database')) {
    fs.mkdirSync(process.cwd() + '/Leo_Database');
    fs.writeFileSync(process.cwd() + '/Leo_Database/A_README.md', 'Thư mục này được ChernobyL(NANI =)) ) sử dụng để lưu trữ dữ liệu. Không xóa thư mục này hoặc bất kỳ tập tin nào trong đó.', 'utf8');
}
var db = new BetterDB(process.cwd() + "/Leo_Database/SyntheticDatabase.sqlite");

function Lset(key, value) {
    if (!key)
        throw new TypeError(
            "Không có khóa nào được chỉ định."
        );
