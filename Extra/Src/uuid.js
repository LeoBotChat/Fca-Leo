...
class UUIDAPIKey {
  ...
  isUUID(uuid) {
    if (!uuid) {
      throw new ReferenceError('Tham số bắt buộc \'uuid\' không được xác định.');
    }
  ...
  isAPIKey(apiKey) {
    if (!apiKey) {
      throw new ReferenceError('Tham số bắt buộc \'apiKey\' chưa được xác định.');
    }
  ...
  toAPIKey(uuid, options) {
    ...
    throw new TypeError(`Giá trị được cung cấp '${uuid}' không phải là uuid hợp lệ.`);
  }
  ...
  toUUID(apiKey) {
    ...
    throw new TypeError(`Giá trị được cung cấp '${apiKey}' không phải là Khóa api hợp lệ.`);
  }
  ...
  check(apiKey, uuid) {
    ...
    let errMsg = '';
    if (!apiTest) {
      errMsg += `Giá trị được cung cấp '${apiKey}' không phải là Khóa api hợp lệ. `;
    }
    if (!uuidTest) {
      errMsg += `Giá trị được cung cấp '${uuid}' không phải là uuid hợp lệ. `;
    }
