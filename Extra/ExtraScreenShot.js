...
const scrollToElement = (element, options) => {
	...
	const calculateOffset = (rect, options) => {
		...
		switch (options.offsetFrom) {
			...
			default:
				throw new Error('Giá trị `scrollToElement.offsetFrom` không hợp lệ!');
		}
	};
...
const captureWebsite = async (input, options) => {
	...
	if (options.emulateDevice) {
		if (!(options.emulateDevice in puppeteer.devices)) {
			throw new Error(`Tên thiết bị \`${options.emulateDevice}\` không được hỗ trợ!`);
		}
	...
	if (options.inset && !screenshotOptions.fullPage) {
		...
		if (width === 0 || height === 0) {
			...
			throw new Error('Khi sử dụng tùy chọn `clip`, chiều rộng hoặc chiều cao của ảnh chụp màn hình không được bằng 0.');
		}
